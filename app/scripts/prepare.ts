...
async function stubIndexHtml() {
  const views = ['background', 'devtools'];
  ...
  for (const view of views) {
    ...
    data = data
      .replace(
        '"./main.tsx"',
        `"http://localhost:${port}/pages/${view}/main.tsx"`,
      )
      .replace(
        '"./main.ts"',
        `"http://localhost:${port}/pages/${view}/main.ts"`,
      )
      .replace(
        '"./index.ts"',
        `"http://localhost:${port}/pages/${view}/index.ts"`,
      )
      .replace(
        '"./index.tsx"',
        `"http://localhost:${port}/pages/${view}/index.tsx"`,
      )
      .replace(
        '<div id="app"></div>',
  ...
  data = data
    .replace(
      '"./panel.tsx"',
      `"http://localhost:${port}/pages/devtools/panel.tsx"`,
    )
    .replace(
      '<div id="app"></div>',
      '<div id="app">Vite server did not start</div>',
